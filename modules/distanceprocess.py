...
def calculate_distance(base_folder, site_names):
    ...
    for site in site_names:
        df_site_ho = df_ho['eNodeB Name'].str[:8]
        if df_site_ho.empty:
        ...
        source_cells = df_cp[df_cp['CellName'] == site]
        ...
        try:
            ...
            for _, row in df_site_ho.iterrows():
                neighbor_name = str(row['DEST Cell Name'])[:8]
                neighbor_cells = df_cp[df_cp['Site_name'].str.startswith(neighbor_name)]
                ho_date = row['Date']
                hho_count = row['VS.HHO.AttOut.NCell']
